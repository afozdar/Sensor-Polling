...
def run(data):
   ...
   #auto adjust y-max to max value * 10% buffer 
    for i in range(1,len(data)):
        pos = allaxes[i-1]
        if float(data[i]) > float( maxyvals[pos]):
            current_axis = axes[pos]
            logging.info(("Growing Y-Axis Max for Axis: {}").format(pos))
            new_max = float(data[i])*1.10
            current_axis.set_ylim(-2, new_max)
            maxyvals[pos]=data[i]        
            current_axis.figure.canvas.draw()
...
def init_graph():
    ...
    axespos = [] #list of each sensor's axis number without duplicates
    allaxes=[] #list of all sensors axis with duplicates
    ...
    for i in range (0,len(st.sensorinfo["sensors"])):
    	...
    	allaxes.append(int(st.sensorinfo["sensors"][i]["graphnumber"]))
...
def graph_data():
    ...
    graphani = animation.FuncAnimation(fig, run, data_gen, blit=True, interval=10000, repeat=False)
    plt.show()
